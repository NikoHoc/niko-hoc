...
const About = () => {
    ...
    // 2. Variant untuk kolom Profile (Kiri)
    const leftColumnVariants: Variants = {
        hidden: { opacity: 0, x: -50 },
        visible: { opacity: 1, x: 0, transition: { duration: 0.8, ease: "easeOut" } }
    };

    // 3. Variant untuk kolom Info & Interests (Kanan) beserta anak-anaknya
    const rightContainerVariants: Variants = {
        hidden: { opacity: 0 },
        visible: {
            opacity: 1,
            transition: {
                staggerChildren: 0.15, // Jeda waktu kemunculan tiap card interest
            }
        }
    };
    ...
    return (
        <section id="about" className="py-20 flex flex-col items-start gap-4 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 overflow-hidden">
            ...
            <motion.div 
                ...
                initial="hidden"
                whileInView="visible"
                viewport={{ once: true, amount: 0.5 }} // Animasi jalan saat 20% elemen masuk layar. once: true agar tidak ngulang terus.
            ...
            <div className="flex flex-col lg:flex-row gap-10 lg:gap-16 w-full items-stretch">
                ...
                {/* RIGHT COLUMN: WHO AM I & INTERESTS */}
                <motion.div 
                    className="w-full lg:w-3/5 flex flex-col gap-8 h-full justify-start"
                    initial="hidden"
                    whileInView="visible"
                    viewport={{ once: true, amount: 0.5 }}
                ...
                >
                    ...
                    <div className="flex flex-col gap-3">
                        <motion.h3 variants={rightItemVariants} className="text-2xl font-bold text-white">My Interests</motion.h3>
                        <div className="flex flex-col gap-4">
                            {/* Card Interests di-map dan diberikan animasi satu-satu */}
                            {personal.interests.map((interest, index) => (
                                <motion.div 
                                    key={index} 
                                    variants={rightItemVariants}
                                    className="flex items-start gap-4 bg-neutral-900 border border-neutral-800 p-4 rounded-xl hover:border-blue-500/30 transition-colors"
                                >
            ...
            <motion.div 
                className="w-full mt-4 bg-neutral-900 border border-neutral-800 rounded-2xl py-3 overflow-hidden relative"
